...
var orderControl = require('./controllers/order');
app.use('/order', orderControl);
orderControl.params = { configHeader: configHeader, configDB: configDB};
...
/// ..................................................
app.get('/product/create', createProductPage);
function createProductPage(req, res) {
    if (session.user) {
        if (req.query.username && req.query.username.trim() != "") {
            ...
            session.user = accsubmit;
            libDB.res_insertDB(MongoClient, urldb, "atnshop", "product",
                accsubmit, "pages/product_create", {title: "ATN-Shop create PRODUCT page" , configHeader: configHeader , currpage: "Product create"}, "Notify", res );
            console.log("\t create ", accsubmit);
        } else {
            res.render("pages/product_create", {title: "ATN-Shop create PRODUCT page", Notify: "", configHeader: configHeader , currpage: "Product create" });
        }
...
function qrPage(req, res) {
    ...
    for(var key in inter) {
        if (key.indexOf("Wi-Fi") >= 0) {             
            ...
            str = "https://www.facebook.com/theshopatn/";
            sv = new QRCode({
                content: str,
                padding: 4,
                width: 512,
